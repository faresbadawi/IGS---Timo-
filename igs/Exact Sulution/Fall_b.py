...
x = np.linspace(0, 1, 100)  # Hier können Sie den Bereich und die Anzahl der Punkte anpassen

# Berechnung der Funktion w(x)
w = -(1 / (24 * K_b)) * (x**4 - 2 * x**3 + x**2) - (1 / (2 * K_s)) * (-x**2 + x)
...
ax.set_title('Eingespannter Träger')  # Titel

# Markierung des Minimums als Punkt mit gerundetem Wert
ax.scatter(min_x, min_w, color='red', label=f'Min: {rounded_min_w:.5f}')  # min_x statt min_w
ax.legend()
...
plt.savefig(r'C:\Users\Fares Badawi\Desktop\4. Semester\Studienarbeit\Bilder\Exact sultion\Exact_solution_b.png', dpi=500, bbox_inches='tight')
plt.show()
